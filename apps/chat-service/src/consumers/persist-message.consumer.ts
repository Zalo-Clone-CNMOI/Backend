...
export class PersistMessageConsumer {
  ...
  @EventPattern(KafkaTopics.ChatInviteMessageCreated)
  async onInviteMessage(@Payload() payload: ChatInviteMessageCommand) {
    const traceId = payload.trace_id;
    ...
    if (!acquired) {
      this.shared.logger.debug(
        `[${traceId}] Invite message already processed (idempotent)`,
      );
      return;
    }

    try {
      await this.repo.insertInviteMessage({
        message_id: payload.message_id,
        conversation_id: payload.conversation_id,
      ...
      this.shared.logger.log(`[${traceId}] Invite message persisted`);
    } catch (error) {
      await this.repo
        .clearMessageProcessing(payload.message_id)
        .catch(() => {});
      ...
      if (this.shared.isNonRetryableBindError(error)) {
        this.shared.logPoisonPayload('ChatInviteMessage', traceId, {
          messageId: payload.message_id,
          conversationId: payload.conversation_id,
          createdAt: payload.created_at,
          reason: 'non_retryable_bind_error',
      ...
      this.shared.logger.error(`[${traceId}] ChatInviteMessage failed`, {
        messageId: payload.message_id,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
  ...
  @EventPattern(KafkaTopics.ChatInviteMessageUpdated)
  async onInviteMessageUpdated(
    @Payload() payload: ChatInviteMessageUpdatedEvent,
  ) {
    const traceId = payload.trace_id;
    ...
    try {
      ...
      if (!msgInfo) {
        this.shared.logger.warn(
          `[${traceId}] Invite message not found for update`,
          {
            messageId: payload.message_id,
          },
        );
      ...
      this.shared.logger.log(`[${traceId}] Invite message metadata updated`);
    } catch (error) {
      this.shared.logger.error(`[${traceId}] ChatInviteMessageUpdated failed`, {
        messageId: payload.message_id,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
