      ...
      const source$ = this.kafka.emit(topic, payload).pipe(
        ...
        retry({
          ...
          delay: (error, retryCount) => {
            this.logger.warn(
              `Retry ${retryCount} for topic ${topic} in ChatPublisher due to error: ${error instanceof Error ? error.message : String(error)}`,
            );
        ...
        catchError((err) => {
          this.logger.error(
            `Failed completely to send to ${topic} in ChatPublisher after ${MAX_RETRIES} attempts.`,
          );
          return throwError(
            () =>
              new Error(
