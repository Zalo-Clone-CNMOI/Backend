...
async function bootstrap() {
  ...
  try {
    ...
    if ((await manager.count(Conversation)) === 0 && users.length >= 6) {
      ...
      const directConv2 = await manager.save(
        manager.create(Conversation, {
          type: ConversationType.DIRECT,
      ...
      const groupConv = await manager.save(
        manager.create(Conversation, {
          type: ConversationType.GROUP,
          name: 'Tech Talk',
          createdById: users[1].id,
          avatarUrl:
            'https://ui-avatars.com/api/?name=Tech+Talk&background=random',
        }),
      );
      await manager.save(ConversationMember, [
        manager.create(ConversationMember, {
      ...
      const weekendConv = await manager.save(
        manager.create(Conversation, {
          type: ConversationType.GROUP,
          name: 'Weekend Plans',
          createdById: users[4].id,
          avatarUrl:
            'https://ui-avatars.com/api/?name=Weekend+Plans&background=random',
        }),
      );
      await manager.save(ConversationMember, [
        manager.create(ConversationMember, {
          conversationId: weekendConv.id,
          userId: users[4].id,
          role: UpdateMemberRoleDtoRoleEnum.OWNER,
        }),
        manager.create(ConversationMember, {
    ...
    if ((await manager.count(Post)) === 0 && users.length >= 9) {
      ...
      const comment2 = await manager.save(
        manager.create(PostComment, {
          postId: p3.id,
          userId: users[1].id,
      ...
      // Reply to comment2
      const comment3 = await manager.save(
        manager.create(PostComment, {
          postId: p3.id,
          userId: users[2].id,
